...
module.exports = {
  ...
  generateSignature: function(keyPair, content) {
    ...
    return {
      ...
      sig: util.b64enc(bytesToBuffer(sig)),
      jwk: keyPair.publicKey
    };
  },

  verifySignature: function(sig, content) {
    ...
    var signatureInputBuf = Buffer.concat([util.b64dec(sig.nonce), content]);
    var signatureInput = bufferToBytes(signatureInputBuf);
    ...
    var publicKey = importPublicKey(sig.jwk);
    var mysig = bufferToBytes(util.b64dec(sig.sig));

    return publicKey.verify(md.digest().bytes(), mysig);
  },
  ...
  generateCertificate: function(ca, serialNumber, csr_b64) {
    ...
    var base64DER = forge.asn1.toDer(forge.pki.certificateToAsn1(cert));
    return util.b64enc(bytesToBuffer(base64DER));
  },
