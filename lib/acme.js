  ...
  function handleAuthz(method, path, headers, body) {
    ...
    switch (method) {
      ...
      case "POST":
        ...
        for (var key in authz.challenges) {
    ...
    authorizeKeyPair: function(keyPair, domain, callback) {
      ...
      function handleChallenge(code, header, body) {
        ...
        if ("simpleHttps" in authz.challenges) {
          challenge = authz.challenges.simpleHttps;
          response = SimpleHttpsResponse(challenge);
          ...
          challenge = authz.challenges.dvsni;
          response = DvsniResponse(challenge);
...
module.exports = {
  ...
  getMeACertificate: function(acmeServer, domain, callback) {
    // Create a client for this host and some key pairs
    var authzURL = "https://" + acmeServer + "/acme/new-authz";
    var certURL = "https://" + acmeServer + "/acme/new-cert";
    var client = this.createClient(authzURL, certURL);
